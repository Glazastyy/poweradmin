...
abstract class BaseController
{
    ...
    /**
     * Whether the configured session key is one an attacker could guess.
     *
     ...
     */
    private static function isWeakSessionKey(mixed $key): bool
    {
        if (!is_string($key)) {
            return true;
        }

        return strlen($key) < self::MIN_SESSION_KEY_LENGTH
            || in_array($key, ['p0w3r4dm1n', 'change_this_key'], true);
    }
